...
trait HListUtils2[Appendable, AppendablePositive[_, _ <: Appendable] <: Appendable, AppendZero <: Appendable] {
  ...
  def abab[M1[_, _, _], M2[_], M3[_], M4[_]]: CCDD.ABDECD[
    Appendable,
    ...
    AppendZero,
    ({ type MX[A, B, C] = (SimpleProduct.AppendMonad[M1], SimpleProduct.TypeGen[M1, M2, M3, M4]) => M1[A, B, C] })#MX
  ] =
    new CCDD.ABDECD[
      Appendable,
      ({ type H1[Head, Tail <: Appendable] = AppendablePositive[M2[Head], Tail] })#H1,
      ...
      override def append[A, P <: Appendable, X <: Appendable, Q <: Appendable](
        m: (SimpleProduct.AppendMonad[M1], SimpleProduct.TypeGen[M1, M2, M3, M4]) => M1[P, X, Q]
      ): (
        SimpleProduct.AppendMonad[M1],
        SimpleProduct.TypeGen[M1, M2, M3, M4]
      ...
      override def zero
        : (SimpleProduct.AppendMonad[M1], SimpleProduct.TypeGen[M1, M2, M3, M4]) => M1[AppendZero, AppendZero, AppendZero] = { (o, func) =>
        o.to(o.zero)(
