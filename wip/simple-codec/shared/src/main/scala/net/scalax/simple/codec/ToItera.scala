...
object ToItera {
  ...
  @inline private def derivedDevImpl[T, F[_[_]]](
    basedInstalled: SimpleProduct3.NotHList.Appender[F]
  ): SimpleProduct3.NotHList.Appender[({ type F1[TX[_]] = F[({ type T1[_] = TX[T] })#T1] })#F1] =
  ...
  @inline private def derivedProductImpl[T, F[_[_]]](
    basedInstalled: SimpleProduct3.NotHList.Appender[F]
  ): SimpleProduct3.NotHList.Appender[({ type F1[TX[_]] = F[({ type T1[_] = TX[T] })#T1] })#F1] =
