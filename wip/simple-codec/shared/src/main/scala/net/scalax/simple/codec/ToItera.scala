...
object ToItera {
  ...
  @inline private def derivedDevImpl[F[_[_]]](basedInstalled: SimpleProduct3.NotHList.Appender[F]): ToItera[F] = new ToItera[F] {
  ...
  @inline private def derivedProductImpl[F[_[_]]](basedInstalled: SimpleProduct3.NotHList.Appender[F]): ToItera[F] = new ToItera[F] {
    override def to[T]: SimpleProduct3.NotHList.Appender[({ type F1[TX[_]] = F[({ type T1[_] = TX[T] })#T1] })#F1] =
      basedInstalled.asInstanceOf[SimpleProduct3.NotHList.Appender[({ type F1[TX[_]] = F[({ type T1[_] = TX[T] })#T1] })#F1]]
  }

  class ApplyImpl[F[_[_]]](codingEnv: CodingEnv) {

    def derived(implicit basedInstalled: SimpleProduct3.NotHList.Appender[F]): ToItera[F] =
      codingEnv.fold(dev = derivedDevImpl[F](basedInstalled), product = derivedProductImpl[F](basedInstalled))

  }

  def apply[F[_[_]]](implicit codingEnv: CodingEnv): ApplyImpl[F] = new ApplyImpl[F](codingEnv = codingEnv)
