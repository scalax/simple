...
object RunTest1 {
  ...
  def build(分子: Long, 分母: Long): ghdmzsk = {
    def buildImpl(isFenmu: Boolean, numLong: Long, zero: () => ghdmzsk): ghdmzsk = {
      if (numLong > 0) {
        if (isFenmu) {
          乘除1.被乘数分子.inputGHDMZSK(() => buildImpl(isFenmu = isFenmu, numLong = numLong - 1, zero))
        } else {
          乘除1.`被乘数分母/乘数分母`.inputGHDMZSK(() => buildImpl(isFenmu = isFenmu, numLong = numLong - 1, zero))
        }
      } else {
        zero()
      }
  ...
  def main1(arr: Array[String]): Unit = {
    ...
    val result1: () => ghdmzsk = () => num1.inputGHDMZSK(() => num2).inputGHDMZSK(() => 乘除1.被乘数分子).inputGHDMZSK(() => 乘除1.`被乘数分母/乘数分母`)
    ...
    val result2: () => ghdmzsk = () =>
      result1().inputGHDMZSK(() => num3).inputGHDMZSK(() => 乘除1.`被乘数分子`).inputGHDMZSK(() => 乘除1.`被乘数分母/乘数分母`)
    ...
    val result3: () => ghdmzsk = () => result2().inputGHDMZSK(result1).inputGHDMZSK(() => 乘除1.`被乘数分子`).inputGHDMZSK(() => 乘除1.`被乘数分母/乘数分母`)
    count(result3, except1 = except5, except2 = except6, printlnSum = 18)

    val result4: () => ghdmzsk = () => result2().inputGHDMZSK(result1).inputGHDMZSK(() => 乘除1.`被乘数分母/乘数分母`).inputGHDMZSK(() => 乘除1.`被乘数分子`)
