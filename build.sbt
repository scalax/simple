...
lazy val `adt-ghdmzskJVM` = `adt-core`.jvm dependsOn (ghdmzskJVM, `test-commonJVM` % Test) aggregate ghdmzskJVM
lazy val `adt-ghdmzskJS`  = `adt-core`.js dependsOn (ghdmzskJS, `test-commonJS`    % Test) aggregate ghdmzskJS
...
`adt-codegen` / rootCodegenPath := (`adt-ghdmzskJVM` / baseDirectory).value / ".." / "shared" / "src" / "codegen"
